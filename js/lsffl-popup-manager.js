...
// MFL SCRIPTS POPUPS
// Player and score popups remain enabled; custom franchise popups are disabled.
...
var MFLPopupCommishMessage = "TEST";
...
<script src="https://edmoak.github.io/LSFFL-Webpage/js/lsffl-popup-manager.js?v=1"></script>
...
body.lsffl-refresh #MFLBoxWrapper .lsffl-clean-team img{
  display:block!important;
...
(function(){
  ...
  function abbreviateName(value){
    ...
    /* Preserve abbreviations already supplied by MFL. */
    if(/^[A-Z0-9]{2,5}$/.test(cleaned)){
      return cleaned;
    }
  ...
  function findNames(card,images){
    ...
    return names.slice(0,2).map(abbreviateName);
  }
  ...
  function teamRow(img,name,record){
    ...
    nameEl.textContent = abbreviateName(name || "TEAM");
  ...
  function rebuildCard(card){
    ...
    card.appendChild(teamRow(images[0],names[0],records[0]));
    card.appendChild(teamRow(images[1],names[1],records[1]));
  ...
  function buildScoreboard(){
    ...
    try{
      ...
      /* MFL marks the current-week links as disabled, which dims them. */
  ...
  function start(){
    ...
    observer = new MutationObserver(function(){
      if(!isBuilding){
        /*
          MFL replaces the matchup HTML when the week arrows are used.
          Reapply the LSFFL structure after every replacement.
        */
        scheduleBuild(80);
      }
    });
