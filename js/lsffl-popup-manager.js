...
function popupCurrentFranchiseId(){
  ...
  for(var i=0;i<candidates.length;i++){
    ...
    if(id){
      popupResolvedFranchiseId=id;
      return id;
    }
  }
  return"";
}
function popupResolveCurrentFranchiseId(){
  var existing=popupCurrentFranchiseId();
  if(existing){return Promise.resolve(existing);}

  var url=new URL(MFL_ORIGIN+"/"+YEAR+"/export");
  url.searchParams.set("TYPE","myleagues");
  ...
  return fetch(url.href,{
    ...
    if(!response.ok){throw new Error("myleagues failed "+response.status);}
    return response.json();
  }).then(function(data){
    var root=data&&data.leagues?data.leagues:data;
    var leagues=popupAsArray(root&&root.league);
    for(var i=0;i<leagues.length;i++){
      var leagueId=String(leagues[i].league_id||leagues[i].id||"");
      if(leagueId===String(LEAGUE_ID)){
        var id=normalizeFranchiseId(leagues[i].franchise_id||leagues[i].franchiseId||"");
        if(id&&id!=="0000"){
          popupResolvedFranchiseId=id;
          return id;
        }
...
function popupCurrentFranchiseName(){
  var id=popupCurrentFranchiseId();
  if(!id){return"";}
  try{
    if(window.franchiseNames&&window.franchiseNames[id]){
      return cleanText(window.franchiseNames[id]);
    }
  }catch(error){}
  return"";
}
...
function popupCurrentTeamHasInvalidLineup(){
  ...
  var franchiseName=popupCurrentFranchiseName().toLowerCase();
  if(!franchiseId&&!franchiseName){return false;}

  var rows=Array.prototype.slice.call(
    table.querySelectorAll("tbody tr:not(.invalid-teams):not(.valid-teams):not(.reportfooter)")
  );

  for(var i=0;i<rows.length;i++){
    var rowText=cleanText(rows[i].textContent).toLowerCase();
    if(franchiseName&&(rowText===franchiseName||rowText.indexOf(franchiseName)!==-1)){
      return true;
    }
    ...
    for(var j=0;j<links.length;j++){
      ...
      if(url&&franchiseId&&normalizeFranchiseId(url.searchParams.get("F"))===franchiseId){
        return true;
      }
      var linkText=cleanText(links[j].textContent).toLowerCase();
      if(franchiseName&&linkText===franchiseName){
        return true;
      }
    }
  }
...
function popupLoadOwnerData(){
  return popupResolveCurrentFranchiseId().then(function(franchiseId){
    if(!franchiseId){
...
function loadAnnouncementItems(){
  ...
  if(lineupBad){
    ownerStatusLines.push("⚠ LINEUP ACTION REQUIRED — The LSFFL lineup checker reports your starting lineup is invalid. Fix your lineup before the applicable game locks.");
  }
  if(rosterBad){
